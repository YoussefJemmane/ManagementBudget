<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __("Editer Une Analyse") }}
        </h2>
    </x-slot>

    <div class="flex justify-center py-12 ">
        <div class="border rounded-md bg-white p-[20px]  w-[1000px]">
            <form method="POST" action="{{ route('formulaireanalyse.update', $formulaireanalyse->id) }}" class=" pt-4 w-[900px] mx-auto">
                @csrf
                @method('PUT')
                <div class="grid md:grid-cols-2 md:gap-6">
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <select onchange="updateSemiDesignationOptions()" name="designantion" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-dark dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" id="designation">
                            ...
                            <option value="1" @if ($formulaireanalyse->designantion == 'Analyses chimiques par chromatographie') selected @endif>
                                Analyses chimiques par chromatographie
                            </option>
                            <option value="2" @if ($formulaireanalyse->designantion == 'Spectroscopie') selected @endif>
                                Spectroscopie
                            </option>
                            <option value="3" @if ($formulaireanalyse->designantion == 'Spectrometrie') selected @endif>
                                Spectrometrie
                            </option>
                            <option value="4" @if ($formulaireanalyse->designantion == 'Microscopie') selected @endif>
                                Microscopie
                            </option>
                            <option value="5" @if ($formulaireanalyse->designantion == 'Cristallographie') selected @endif>
                                Cristallographie
                            </option>
                            <option value="6" @if ($formulaireanalyse->designantion == 'Analyses Environnementales') selected @endif>
                ...
                <!-- Mode de Facturation -->
                <div class="relative z-0 w-full mb-5 group">
                    <input id="mode_facturation" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-dark dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" type="text" name="mode_facturation" :value="old('mode_facturation')" required autofocus value="{{ $formulaireanalyse->mode_facturation }}" />
