<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Editer un Service') }}
        </h2>
    </x-slot>

    <div class="flex justify-center py-12 " x-data="{ selectedForm: '' }" @if ($service->type_service == 'traduction') x-init="selectedForm = 'traduction'" @elseif ($service->type_service == 'publication') x-init="selectedForm = 'publication'" @elseif ($service->type_service == 'revision') x-init="selectedForm = 'revision'" @endif>

        <div class="border rounded-md bg-white p-[20px]  w-[1000px]">
            <form action="{{ route('services.update', $service->id) }}" method="POST" enctype="multipart/form-data"  class=" pt-4 w-[900px] mx-auto">
                @csrf
                ...
                <div class="mb-4 flex justify-around pt-3">
                    ...
                    <input type="radio" id="traductionCheckbox" name="service" value="traduction"
                           @change="selectedForm = $event.target.checked ? 'traduction' : ''" @if ($service->type_service == 'traduction') checked @endif>
                    <label for="traductionCheckbox">Traduction</label><br>
                    <input type="radio" id="publicationCheckbox" name="service" value="publication"
                           @change="selectedForm = $event.target.checked ? 'publication' : ''" @if ($service->type_service == 'publication') checked @endif>
                    <label for="publicationCheckbox">Publication</label><br>
                    <input type="radio" id="revisionCheckbox" name="service" value="revision"
                           @change="selectedForm = $event.target.checked ? 'revision' : ''" @if ($service->type_service == 'revision') checked @endif>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input type="text" name="titre" id="titre"
                           ...
                           placeholder=" " required  value="{{ $service->titre }}" autofocus />
                    <label for="titre"
                ...
                <div x-show="selectedForm === 'publication'">
                    ...
                    <div class="grid md:grid-cols-2 md:gap-6">
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            <input id="ISSN" value="{{ $service->ISSN }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-dark dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" type="text" name="ISSN"   autofocus />
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input id="article" value="{{ $service->article }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-dark dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" type="file" name="article"   autofocus />
